    ...
    <section id="projects" className="py-24 bg-gray-900">
      <div className="container mx-auto px-6">
        <SectionHeading eyebrow={t.projects.eyebrow} subtitle={t.projects.subtitle}>
          {t.projects.titleLead}<span className="text-blue-400">{t.projects.titleHighlight}</span>
        </SectionHeading>
        ...
        {!loading && error && (
          <p className="text-center text-gray-500">{t.projects.error}</p>
        )}

        {!loading && !error && projects.length === 0 && (
          <p className="text-center text-gray-500 text-lg">{t.projects.comingSoon}</p>
        )}
        ...
        {!loading && !error && projects.length > 0 && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((p) => (
              <SpotlightCard
                key={p.id}
                className="bg-gray-800 rounded-2xl p-6 shadow-2xl border border-gray-700 hover:border-blue-500 card-hover flex flex-col"
              >
                {p.image_url && (
                  <div className="rounded-xl overflow-hidden mb-4 aspect-video bg-gray-900">
                    <img
                      src={p.image_url}
                      alt={lang === 'en' ? `${p.title_en || p.title} project image` : `Imagem do projeto ${p.title}`}
                      loading="lazy"
